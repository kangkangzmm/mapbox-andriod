package com.yinglan.scrolllayout.demo.core.voip;
...
import com.yinglan.scrolllayout.demo.permission.Permissions;
...
public class VoipReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Consts.ACTION_VOIP_RECEIVER.equals(action)) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                if (com.yinglan.scrolllayout.demo.core.util.Utils.isAppRunningForeground()) {
                ...
                onForegroundOrBeforeVersionO(
                        ...
                        audioOnly,
                        inviteUserName,
                        com.yinglan.scrolllayout.demo.core.util.Utils.isAppRunningForeground()
                );
            }
        }
    }
    ...
    private void onBackgroundAfterVersionO(
        ...
        for (String str : strArr)
            list.add(str);
        SkyEngineKit.init(new VoipEvent());
        RtcBaseActivity activity = (RtcBaseActivity) ActivityStackManager.getInstance().getTopActivity();
        // 权限检测
        String[] per;
        if (audioOnly) {
        ...
        if (hasPermission) {
            ...
            CallForegroundNotification notification = new CallForegroundNotification(AppVdeo.getInstance());
    ...
    private void onBackgroundHasPermission(
            ...
            String inviteId, Boolean audioOnly, String inviteUserName) {
        boolean b = SkyEngineKit.Instance().startInCall(AppVdeo.getInstance(), room, inviteId, audioOnly);
        Log.d(TAG, "onBackgroundHasPermission b = " + b );
        if (b) {
            AppVdeo.getInstance().setOtherUserId(inviteId);
            if (list.size() == 1) {
                CallForegroundNotification notification = new CallForegroundNotification(AppVdeo.getInstance());
    ...
    private void onPermissionDenied(String room, String inviteId) {
        ...
        Toast.makeText(AppVdeo.getInstance(), "权限被拒绝，无法通话", Toast.LENGTH_SHORT).show();
    }

    private void shouldStartRing(boolean isComing) {
        if (isComing) {
            Uri uri = Uri.parse("android.resource://" + AppVdeo.getInstance().getPackageName() + "/" + R.raw.incoming_call_ring);
            ringPlayer.play(AppVdeo.getInstance(), uri, true, AudioManager.STREAM_RING);
        } else {
            Uri uri = Uri.parse("android.resource://" + AppVdeo.getInstance().getPackageName() + "/" + R.raw.wr_ringback);
            ringPlayer.play(AppVdeo.getInstance(), uri, true, AudioManager.STREAM_RING);
        }
    }
