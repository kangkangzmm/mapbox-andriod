 ...
 * 单人通话界面
 */
public class CallSingleActivity extends RtcBaseActivity implements CallSession.CallSessionCallback {
...
//        }
    ...
    private void init(String targetId, boolean outgoing, boolean audioOnly, boolean isReplace) {
        ...
        if (outgoing && !isReplace) {
            ...
            AppVdeo.getInstance().setRoomId(room);
            AppVdeo.getInstance().setOtherUserId(targetId);
    ...
    public void didCallEndWithReason(EnumType.CallEndReason reason) {
        AppVdeo.getInstance().setOtherUserId("0");
        //交给fragment去finish
//        finish();
